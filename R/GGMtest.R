...
GGMtest <- function(data = X,
  ...
  for (num_fold in 1:k_fold){
    ...
    lambda_1 <- penalty$c*1/(2*sqrt(n1))*qnorm(1-gamma/(2*p1*max((p-1),n1))) #penalty term eta one
    lambda_2 <- penalty$c*1/(2*sqrt(n1))*qnorm(1-gamma/(2*p1*max((p-2),n1))) #penalty term eta two
    ...
    for (i in block1){ # estimation relevant coefficients
      ...
      for (j in block2){
        ...
        beta1 <- uniroot(function(x) mean(Score(x,X[sample2_index,j],pred1[count,1:n2,num_fold],pred2[count,1:n2,num_fold])),interval = c(-10,10))
  ...
  for (b in 1:nbootstrap){
    for (j in 1:p1){
      if (num_drop == 0){
        epsilon <- matrix(rnorm(n+k_fold),byrow=F,ncol=k_fold)
      } else {
        epsilon <- matrix(rnorm(ceiling(n/k_fold)*k_fold),byrow=F,ncol=k_fold)
      }
  ...
  quant_est <- quantile(nsample,probs = c(1-alpha,1-alpha/2,alpha/2))
  ...
  result <- list(estimates = beta_vec,edge_list = S, hyp_max=hyp_max, hyp_sphere= hyp_sphere,alpha = alpha,
                 nbootstrap = nbootstrap, nuisance_estimaton =  nuisance_estimaton, penalty = penalty, folds = folds,
                 Nsample = Nsample, target_est = target_est, pvalue_max = pvalue_max, pvalue_sphere = pvalue_sphere,
